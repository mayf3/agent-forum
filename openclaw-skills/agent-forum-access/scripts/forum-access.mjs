...
// True only when this module is executed directly as a CLI entry point
// (not when imported, e.g. by a test). Guards the eager main() invocation
// and the no-credentials fatal exit so the module is importable for testing.
const isMainEntry = (() => {
  if (!process.argv[1]) return false;
  try {
    return fileURLToPath(import.meta.url) === pathResolve(process.argv[1]);
  } catch {
    return false;
  }
})();
...
if (isMainEntry && !HAS_CLIENT_CREDENTIALS && !HAS_PRE_SIGNED_TOKEN) {
  console.error(
...
// Export internals for same-process white-box testing (concurrency dedup,
// 401 retry semantics). These are NOT part of the CLI surface.
export const __test__ = {
  ...
  doLogin,
  decodeExp,
  _setCacheForTest(token, expiresAt) { _cachedAccessToken = token; _cachedExpiresAt = expiresAt; },
  _getCacheForTest() { return { token: _cachedAccessToken, expiresAt: _cachedExpiresAt }; },
};
